...
def parse_file(file_obj, parser_class=None, single_point_only=False):
    ...
    while lineno < lines_length:
        ...
        if section == "output":
            ...
            optimise_start = re.findall(
                "Start of (bulk|surface|polymer) optimisation", line
            )
            if optimise_start:
        ...
        if section == "optimisation":
            ...
            if line.strip().startswith("Final energy"):
                ...
                section = "post_opt"
                output["parser_errors"].append(
                    "Reached final energy, before finding 'Optimisation achieved'"
                )
                continue
        ...
        if section == "output":
            ...
            if line.strip().startswith("Components of energy :"):
                energy, penergy = (
                    ...
                    else ("initial_energy", "initial_primitive_energy")
                )
                try:
        ...
        if section == "output" or section == "optimisation":
            ...
            if line.strip().startswith("ReaxFF : Energy contributions:"):
                ...
                try:
                    output["energy_contributions"], lineno = read_reaxff_econtribs(
                        lines, lineno
                    )
                except (IOError, ValueError) as err:
                    output["parser_errors"].append(str(err))
        ...
        if section == "output" or section == "post_opt":
            ...
            if line.strip().startswith("Time to end of optimisation"):
                ...
                time_match = re.findall(
                    "Time to end of optimisation[\\s]*=[\\s]*([+-]?[0-9]*[.]?[0-9]+) seconds",
                    line,
                )
                if time_match:
