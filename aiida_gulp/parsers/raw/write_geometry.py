...
import numpy as np
from aiida_gulp.symmetry import (
    convert_structure,
...
def create_geometry_lines(structure_data, symmetry_data=None, name="main-geometry"):
    ...
    if symmetry_data is not None:
        ...
        if atoms.get_number_of_atoms() != len(equivalent):
            raise ValueError(
                "number of atomic sites != number of symmetry equivalent sites"
            )
        used_equivalents = []
        for site, eq in zip(atoms, equivalent):
            if eq not in used_equivalents:
                lines.append(
                    "{0} core {1:.6f} {2:.6f} {3:.6f}".format(
        ...
        for site in atoms:
            lines.append(
                "{0} core {1:.6f} {2:.6f} {3:.6f}".format(site.symbol, *site.position)
            )
    ...
    if symmetry_data is not None:
        ...
        for op in operations:
            ...
            lines.append(
                "{0:8.5f} {1:8.5f} {2:8.5f} {3:8.5f}".format(op[0], op[3], op[6], op[9])
            )
            lines.append(
                "{0:8.5f} {1:8.5f} {2:8.5f} {3:8.5f}".format(
...
def validate_1d_geometry(structure):
    ...
    if not list(structure.pbc) == [True, False, False]:
        raise NotImplementedError(
            "a 1-D structure can only be periodic in the x-direction"
        )
