...
from aiida_gulp.validation import load_schema, validate_against_schema
from aiida_gulp.parsers.raw.write_input_fitting import create_input_lines
...
class GulpFittingCalculation(CalcJob):
    ...
    def validate_potential(cls, potential):
        assert (
            potential.has_fitting_flags
        ), "fitting flags should be set for the potential"
    ...
    def define(cls, spec):
        ...
        spec.input(
            "metadata.options.input_file_name",
            valid_type=six.string_types,
            default="main.gin",
        )
        spec.input(
            "metadata.options.output_main_file_name",
            valid_type=six.string_types,
            default="main.gout",
        )
        spec.input(
            "metadata.options.output_stderr_file_name",
            valid_type=six.string_types,
            default="main_stderr.txt",
        )
        spec.input(
            "metadata.options.output_dump_file_name",
            valid_type=six.string_types,
            default="fitting.grs",
        )
        spec.input(
            "metadata.options.allow_create_potential_fail",
            valid_type=bool,
            default=False,
        )
        spec.input(
        ...
        spec.input(
            ...
            help=(
                "a dictionary defining the potential. "
        ...
        spec.exit_code(
            ...
            message=(
                "An error was flagged trying to parse the " "gulp exec stdout file"
            ),
        )
        ...
        spec.exit_code(
            ...
            message=(
                "The number of fitting variables exceeds the number of observables"
            ),
        )
        spec.exit_code(
            311, "ERROR_FIT_UNSUCCESFUL", message=("The fit was not successful")
        )
        spec.exit_code(
            312,
            "ERROR_GULP_UNKNOWN",
            message=(
                "An error was flagged by GULP, which is not accounted for in another exit code"
            ),
        )
        spec.exit_code(
            313,
    ...
    def create_observable_map(self, settings):
        ...
        if "energy" in observables:
            units = observables["energy"].get(
                "units", self._observable_defaults["energy_units"]
            )
            units_key = observables["energy"].get(
                "units_key", self._observable_defaults["energy_units_key"]
            )
            energy_key = observables["energy"].get(
                "energy_key", self._observable_defaults["energy_key"]
            )
            weighting = observables["energy"].get(
                "weighting", self._observable_defaults["weighting"]
            )
            if units == "eV":
                key = "energy ev"
            else:
                ...
                for key in [units_key, energy_key]:
                    if key not in dct:
                        raise AssertionError(
                            "the observable data Pk={0} does not contain a '{1}' key".format(
                                data.id, key
                            )
                        )
